"""Download and organize five public waste datasets into a six-class archive.
...
def main():
    # A failed prior run may have left a partial corpus without a manifest. Rebuild the
    # generated trees from scratch while retaining the download cache, so reruns cannot
    # mix stale files with the pinned five-source corpus.
    for generated in (OUTPUT_ROOT / "included", OUTPUT_ROOT / "excluded"):
        if generated.exists():
