...
def train_run(cfg: dict, val_fold: int = 0, resume: bool = False) -> dict:
    ...
    # fp16 needs a scaler to avoid gradient underflow; bf16 has fp32's exponent range
    # and must NOT be scaled.
    scaler = torch.amp.GradScaler() if amp_dtype is torch.float16 else None
    if cfg.get("compile"):
        # Opt-in: 1.2-2x on modern GPUs, but the first step pays a long compile and a few
        # timm backbones still fall back to eager. Verify with a short run before trusting.
        print("torch.compile enabled — expect a slow first epoch")
